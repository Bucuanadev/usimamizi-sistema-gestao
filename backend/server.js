  ...
  try {
    ...
    produtos.forEach(produtoVendido => {
      ...
      if (produtoIndex !== -1) {
        ...
        // Adicionar à lista de produtos atualizados para log
    ...
    // Adicionar atividade ao log (se não existir, criar)
    if (!data.atividades) {
      data.atividades = [];
    }
    ...
    const atividade = {
      ...
      tipo: tipo_operacao === 'venda' ? 'venda' : tipo_operacao === 'expedicao' ? 'expedicao' : 'entrada',
      descricao: `${tipo_operacao === 'venda' ? 'Venda' : tipo_operacao === 'expedicao' ? 'Expedição' : 'Entrada'} de ${produtosAtualizados.length} produto(s)`,
      detalhes: produtosAtualizados,
      data: new Date().toISOString(),
      status: 'concluido'
    };
    
    data.atividades.unshift(atividade); // Adicionar no início
    if (data.atividades.length > 100) {
      data.atividades = data.atividades.slice(0, 100); // Manter apenas os últimos 100
    }
    ...
    res.json({
      ...
      produtos_atualizados: produtosAtualizados.length,
      detalhes: produtosAtualizados
    });
  } catch (error) {
...
// GET /api/sales/produto-por-codigo/:codigo - Buscar produto por código para preenchimento automático
app.get('/api/sales/produto-por-codigo/:codigo', (req, res) => {
  try {
    const { codigo } = req.params;
    console.log('Buscando produto com código:', codigo);
    const data = readData();
    console.log('Total de produtos encontrados:', data.produtos?.length || 0);
    ...
    console.log('Produto encontrado:', produto ? produto.nome : 'Nenhum');
    ...
    // Buscar preço de venda ativo
    const precoVenda = produto.precos?.find(p => p.tipo === 'venda' && p.ativo);
    
    // Retornar dados formatados para preenchimento automático
    const produtoFormatado = {
      id: produto.id,
      codigo: produto.codigo,
      descricao: produto.nome,
      quantidade: 1, // Quantidade padrão
      preco: precoVenda?.valor || 0,
      desconto: 0, // Desconto padrão
      iva: 0.16, // IVA padrão 16%
      total: precoVenda?.valor || 0,
      produto_id: produto.id,
      stock_disponivel: produto.stock_disponivel || 0,
      categoria_nome: categoria?.nome || 'Sem categoria',
...
app.post('/api/sales/export-orc-pdf', async (req, res) => {
  try {
    ...
    orcData.cliente = orcData.cliente || {};
    orcData.cliente.nome = orcData.cliente.nome || 'Cliente não especificado';
    orcData.cliente.nuit = orcData.cliente.nuit || '';
    orcData.cliente.morada = orcData.cliente.morada || '';
    orcData.document_date = orcData.document_date || new Date().toISOString().split('T')[0];
    orcData.cotacaoItens = orcData.cotacaoItens || [];
    orcData.total_amount = orcData.total_amount || 0;
    orcData.totais = orcData.totais || {};
    orcData.totais.subtotal = orcData.totais.subtotal || 0;
    orcData.totais.iva = orcData.totais.iva || 0;
    orcData.totais.total = orcData.totais.total || 0;
...
app.put('/api/purchases/documents/:id', (req, res) => {
  try {
    ...
    const oldDocument = data.purchaseDocuments[documentIndex];
    const newDocument = {
    ...
    // Se for uma Guia de Entrada (GE) e o status mudou para CONFIRMADO
    if (newDocument.document_type === 'GE' && 
        oldDocument.status !== 'CONFIRMADO' && 
        newDocument.status === 'CONFIRMADO') {
      
      console.log('Guia de Entrada confirmada, processando entrada no stock...');
      
      // Processar entrada no stock
      if (newDocument.linhas && Array.isArray(newDocument.linhas)) {
        const produtosAtualizados = [];
        
        newDocument.linhas.forEach(artigo => {
          if (artigo.codigo && artigo.quantidadeRecebida > 0) {
            // Procurar produto existente pelo código
            let produtoIndex = -1;
            if (data.produtos) {
              produtoIndex = data.produtos.findIndex(p => 
                p.codigo.toLowerCase() === artigo.codigo.toLowerCase()
              );
            }
            
            if (produtoIndex !== -1) {
              // Produto existe, atualizar stock
              const produto = data.produtos[produtoIndex];
              const quantidadeAnterior = produto.stock_atual || 0;
              
              produto.stock_atual = (produto.stock_atual || 0) + artigo.quantidadeRecebida;
              produto.stock_disponivel = (produto.stock_disponivel || 0) + artigo.quantidadeRecebida;
              produto.updated_at = new Date().toISOString();
              ...
              if (artigo.precoUnitario > 0) {
                ...
                // Procurar preço de compra existente
                const precoCompraIndex = produto.precos.findIndex(p => p.tipo === 'compra' && p.ativo);
                
                if (precoCompraIndex !== -1) {
                  produto.precos[precoCompraIndex].valor = artigo.precoUnitario;
                  produto.precos[precoCompraIndex].data_inicio = new Date().toISOString().split('T')[0];
                } else {
                  produto.precos.push({
                    id: `preco_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
                    produto_id: produto.id,
                    tipo: 'compra',
                    valor: artigo.precoUnitario,
                    moeda: 'MZN',
              ...
              produtosAtualizados.push({
                ...
                codigo: produto.codigo,
                nome: produto.nome,
                quantidade_entrada: artigo.quantidadeRecebida,
                stock_anterior: quantidadeAnterior,
                stock_atual: produto.stock_atual,
                preco_compra: artigo.precoUnitario || 0
              });
              
              console.log(`Stock atualizado para ${produto.nome}: ${quantidadeAnterior} → ${produto.stock_atual}`);
            } else {
              ...
              const novoProduto = {
                ...
                codigo_barras: '',
                nome: artigo.descricao || artigo.codigo,
                descricao: artigo.descricao || '',
                categoria_id: 'categoria_padrao', // Categoria padrão para produtos criados via guia de entrada
                unidade_medida: 'un',
                peso: null,
                dimensoes: '',
                stock_atual: artigo.quantidadeRecebida,
                stock_disponivel: artigo.quantidadeRecebida,
                stock_reservado: 0,
                stock_minimo: 0,
                stock_maximo: 1000,
                localizacao: artigo.localizacao || 'Armazém Central',
                fornecedor_principal: newDocument.supplier_name || '',
                observacoes: `Criado via Guia de Entrada ${newDocument.full_number}`,
                ativo: true,
                created_at: new Date().toISOString(),
                updated_at: new Date().toISOString(),
                precos: []
              ...
              // Adicionar preço de compra se fornecido
              if (artigo.precoUnitario > 0) {
                novoProduto.precos.push({
                  id: `preco_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
              ...
              produtosAtualizados.push({
                ...
                quantidade_entrada: artigo.quantidadeRecebida,
                stock_anterior: 0,
                stock_atual: novoProduto.stock_atual,
                preco_compra: artigo.precoUnitario || 0,
                novo_produto: true
              });
              
              console.log(`Novo produto criado: ${novoProduto.nome} com stock ${novoProduto.stock_atual}`);
            }
          }
        });
        ...
        const atividade = {
          id: `atividade_${Date.now()}`,
          tipo: 'entrada_stock',
          descricao: `Entrada no stock via Guia de Entrada ${newDocument.full_number}`,
          detalhes: {
            guia_entrada: newDocument.full_number,
            fornecedor: newDocument.supplier_name,
            produtos_atualizados: produtosAtualizados.length,
        ...
        data.atividades.unshift(atividade);
        if (data.atividades.length > 100) {
          data.atividades = data.atividades.slice(0, 100);
        }
        
        console.log(`Entrada no stock processada: ${produtosAtualizados.length} produto(s) atualizado(s)`);
      }
    }
    
    data.purchaseDocuments[documentIndex] = newDocument;
...
// POST /api/purchases/confirmar-guia-entrada/:id - Confirmar Guia de Entrada e processar entrada no stock
app.post('/api/purchases/confirmar-guia-entrada/:id', (req, res) => {
  try {
    const data = readData();
    if (!data.purchaseDocuments) {
      data.purchaseDocuments = [];
    ...
    const document = data.purchaseDocuments[documentIndex];
    
    if (document.document_type !== 'GE') {
      return res.status(400).json({
        success: false,
        message: 'Documento não é uma Guia de Entrada'
      });
    }
    ...
    if (document.status === 'CONFIRMADO') {
      return res.status(400).json({
        success: false,
        message: 'Guia de Entrada já foi confirmada'
      });
    }
    
    // Atualizar status para CONFIRMADO
    data.purchaseDocuments[documentIndex].status = 'CONFIRMADO';
    data.purchaseDocuments[documentIndex].updated_at = new Date().toISOString();
    
    // Processar entrada no stock
    if (document.linhas && Array.isArray(document.linhas)) {
      const produtosAtualizados = [];
      
      document.linhas.forEach(artigo => {
        if (artigo.codigo && artigo.quantidadeRecebida > 0) {
          // Procurar produto existente pelo código
          let produtoIndex = -1;
          if (data.produtos) {
          ...
          if (produtoIndex !== -1) {
            ...
            const novoProduto = {
              ...
              stock_maximo: 1000,
              localizacao: artigo.localizacao || 'Armazém Central',
              fornecedor_principal: document.supplier_name || '',
              observacoes: `Criado via Guia de Entrada ${document.full_number}`,
              ativo: true,
              created_at: new Date().toISOString(),
              updated_at: new Date().toISOString(),
              precos: []
      ...
      const atividade = {
        id: `atividade_${Date.now()}`,
        tipo: 'entrada_stock',
        descricao: `Entrada no stock via Guia de Entrada ${document.full_number}`,
        detalhes: {
          guia_entrada: document.full_number,
      ...
      if (writeData(data)) {
        res.json({
          success: true,
          data: data.purchaseDocuments[documentIndex],
          message: `Guia de Entrada confirmada e ${produtosAtualizados.length} produto(s) adicionado(s) ao stock`,
      ...
      if (writeData(data)) {
        res.json({
          success: true,
          data: data.purchaseDocuments[documentIndex],
          message: 'Guia de Entrada confirmada (sem produtos para processar)'
        });
      } else {
        res.status(500).json({
          success: false,
...
// GET /api/settings/estabelecimentos - Listar todos os estabelecimentos
app.get('/api/settings/estabelecimentos', (req, res) => {
  try {
    const data = readData();
    const estabelecimentos = data.estabelecimentos || [];
    res.json({ success: true, data: estabelecimentos, total: estabelecimentos.length });
  } catch (error) {
...
// POST /api/settings/estabelecimentos - Criar novo estabelecimento
app.post('/api/settings/estabelecimentos', (req, res) => {
  try {
    const estabelecimentoData = req.body;
    
    // Validações básicas
    if (!estabelecimentoData.codigo || !estabelecimentoData.nome) {
      return res.status(400).json({
        success: false,
    ...
    const codigoExiste = data.estabelecimentos.some(estab => 
      estab.codigo.toLowerCase() === estabelecimentoData.codigo.toLowerCase()
    );
    
    if (codigoExiste) {
      return res.status(400).json({
        success: false,
        message: 'Já existe um estabelecimento com este código'
      });
    }
    ...
    const novoEstabelecimento = {
      ...
      tipo: estabelecimentoData.tipo || 'sucursal',
      nuit: estabelecimentoData.nuit || '',
      endereco: {
        rua: estabelecimentoData.endereco?.rua || '',
        cidade: estabelecimentoData.endereco?.cidade || '',
        provincia: estabelecimentoData.endereco?.provincia || '',
        codigoPostal: estabelecimentoData.endereco?.codigoPostal || '',
        pais: estabelecimentoData.endereco?.pais || 'Moçambique'
      },
      contactos: {
        telefone: estabelecimentoData.contactos?.telefone || '',
        fax: estabelecimentoData.contactos?.fax || '',
        email: estabelecimentoData.contactos?.email || '',
        website: estabelecimentoData.contactos?.website || ''
      },
      responsavel: {
        nome: estabelecimentoData.responsavel?.nome || '',
        cargo: estabelecimentoData.responsavel?.cargo || '',
        telefone: estabelecimentoData.responsavel?.telefone || '',
        email: estabelecimentoData.responsavel?.email || ''
      },
      configuracoes: {
        ativo: estabelecimentoData.configuracoes?.ativo !== false,
        permiteVendas: estabelecimentoData.configuracoes?.permiteVendas !== false,
        permiteCompras: estabelecimentoData.configuracoes?.permiteCompras !== false,
        permiteStock: estabelecimentoData.configuracoes?.permiteStock !== false,
        serieDocumentos: estabelecimentoData.configuracoes?.serieDocumentos || ''
      },
      observacoes: estabelecimentoData.observacoes || '',
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    };

    data.estabelecimentos.push(novoEstabelecimento);
    writeData(data);
...
// PUT /api/settings/estabelecimentos/:id - Atualizar estabelecimento
app.put('/api/settings/estabelecimentos/:id', (req, res) => {
  try {
    const { id } = req.params;
    const estabelecimentoData = req.body;
    ...
    const estabelecimentoIndex = data.estabelecimentos.findIndex(estab => estab.id === id);
    ...
    // Verificar se o código já existe (exceto para o próprio estabelecimento)
    const codigoExiste = data.estabelecimentos.some(estab => 
      estab.id !== id && estab.codigo.toLowerCase() === estabelecimentoData.codigo.toLowerCase()
    );
    ...
    const estabelecimentoAtualizado = {
      ...data.estabelecimentos[estabelecimentoIndex],
      codigo: estabelecimentoData.codigo,
      nome: estabelecimentoData.nome,
    ...
    data.estabelecimentos[estabelecimentoIndex] = estabelecimentoAtualizado;
    writeData(data);
...
// DELETE /api/settings/estabelecimentos/:id - Eliminar estabelecimento
app.delete('/api/settings/estabelecimentos/:id', (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Verificar se é o estabelecimento matriz (não pode ser eliminado)
    const estabelecimento = data.estabelecimentos[estabelecimentoIndex];
    if (estabelecimento.tipo === 'matriz') {
      return res.status(400).json({
        success: false,
        message: 'Não é possível eliminar o estabelecimento matriz'
      });
    }

    data.estabelecimentos.splice(estabelecimentoIndex, 1);
    writeData(data);
...
app.post('/api/sales/export-pdf', async (req, res) => {
  try {
    ...
    // Garantir que os dados essenciais existem
    faturaData.cliente = faturaData.cliente || {};
    faturaData.cliente.nome = faturaData.cliente.nome || 'Cliente não especificado';
    faturaData.cliente.nuit = faturaData.cliente.nuit || '';
    faturaData.cliente.morada = faturaData.cliente.morada || '';
    faturaData.data = faturaData.data || new Date().toISOString().split('T')[0];
    faturaData.linhas = faturaData.linhas || [];
    faturaData.total = faturaData.total || 0;
    faturaData.totais = faturaData.totais || {};
    faturaData.totais.subtotal = faturaData.totais.subtotal || 0;
    faturaData.totais.iva = faturaData.totais.iva || 0;
    faturaData.totais.total = faturaData.totais.total || 0;
    ...
    if (empresa.logotipo && typeof empresa.logotipo === 'string' && empresa.logotipo.startsWith('data:image/')) {
      try {
    ...
    doc.fontSize(4) // Reduzido de 6 para 4 (mais 2 pontos a menos)
       .font('Helvetica-Bold')
       .fillColor('#2c3e50')
       .text(`N°${faturaData.numero ? faturaData.numero.split('/')[1] || faturaData.numero : 'N/A'}`, boxX + 3, boxY + 6); // Ajustado posicionamento
    ...
    doc.font('Helvetica')
       ...
       .text(faturaData.cliente.morada ? faturaData.cliente.morada.split('\n')[0] : '', 100, clientY + 30)
...
app.post('/api/sales/export-gr-pdf', async (req, res) => {
  try {
    ...
    // Garantir que os dados essenciais existem
    grData.cliente = grData.cliente || {};
    grData.cliente.nome = grData.cliente.nome || 'Cliente não especificado';
    grData.cliente.nuit = grData.cliente.nuit || '';
    grData.cliente.morada = grData.cliente.morada || '';
    grData.document_date = grData.document_date || new Date().toISOString().split('T')[0];
    grData.artigos = grData.artigos || [];
    grData.total = grData.total || 0;
    grData.totais = grData.totais || {};
    grData.totais.subtotal = grData.totais.subtotal || 0;
    grData.totais.iva = grData.totais.iva || 0;
    grData.totais.total = grData.totais.total || 0;
...
app.get('/api/dashboard/overview', (req, res) => {
  try {
    ...
    const vendas = data.documents?.filter(doc => 
      doc.document_type === 'FAT' && doc.status === 'SENT'
    ) || [];
    
    const orcamentos = data.documents?.filter(doc => 
      doc.document_type === 'ORC'
    ) || [];
    
    const guias = data.documents?.filter(doc => 
      doc.document_type === 'GUIA'
    ) || [];
    
    // Calcular totais
    const totalVendas = vendas.reduce((sum, venda) => sum + (venda.total_amount || 0), 0);
    const totalOrcamentos = orcamentos.reduce((sum, orc) => sum + (orc.total_amount || 0), 0);
    const totalGuias = guias.reduce((sum, guia) => sum + (guia.total_amount || 0), 0);
    
    // Calcular crescimento mensal
    const mesAtual = new Date().getMonth();
    const vendasMesAtual = vendas.filter(v => 
      new Date(v.document_date).getMonth() === mesAtual
    );
    const vendasMesAnterior = vendas.filter(v => 
      new Date(v.document_date).getMonth() === mesAtual - 1
    );
    ...
    const produtos = data.produtos || [];
    const produtosBaixoStock = produtos.filter(p => 
      p.stock_atual <= p.stock_minimo
    );
    const produtosSemStock = produtos.filter(p => p.stock_atual <= 0);
    ...
    vendas.slice(0, 5).forEach(venda => {
      atividadesRecentes.push({
        id: `venda_${venda.id}`,
        tipo: 'venda',
        titulo: `Fatura ${venda.full_number} emitida`,
        descricao: `Cliente: ${venda.client_name} - ${formatarMoeda(venda.total_amount)}`,
    ...
    orcamentos.slice(0, 3).forEach(orc => {
      atividadesRecentes.push({
        id: `orc_${orc.id}`,
        tipo: 'orcamento',
        titulo: `Orçamento ${orc.full_number} criado`,
        descricao: `Cliente: ${orc.client_name} - ${formatarMoeda(orc.total_amount)}`,
        data: orc.document_date,
        status: 'info',
        icone: 'file-invoice'
      });
    });
    
    // Adicionar alertas de stock
    produtosBaixoStock.slice(0, 3).forEach(produto => {
      atividadesRecentes.push({
        id: `stock_${produto.id}`,
        tipo: 'alerta',
        titulo: `Stock baixo: ${produto.nome}`,
        descricao: `Apenas ${produto.stock_atual} unidades restantes`,
    ...
    res.json({
      ...
      data: {
        resumo: {
          ...
          totalClientes: new Set(vendas.map(v => v.client_name)).size,
        ...
        orcamentos: {
          ...
          pendentes: orcamentos.filter(o => o.status === 'DRAFT').length,
          aprovados: orcamentos.filter(o => o.status === 'APPROVED').length
        },
        stock: {
          totalProdutos: produtos.length,
          baixoStock: produtosBaixoStock.length,
          semStock: produtosSemStock.length,
          valorTotal: produtos.reduce((sum, p) => 
            sum + (p.stock_atual * (p.precos?.find(pr => pr.tipo === 'venda')?.valor || 0)), 0
          )
        },
        atividades: atividadesRecentes.slice(0, 10)
      }
    });
  } catch (error) {
...
// GET /api/dashboard/vendas-chart - Dados para gráfico de vendas
app.get('/api/dashboard/vendas-chart', (req, res) => {
  try {
    const data = readData();
    const vendas = data.documents?.filter(doc => 
      doc.document_type === 'FAT' && doc.status === 'SENT'
    ...
    // Agrupar vendas por mês dos últimos 12 meses
    const vendasPorMes = {};
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      vendasPorMes[mesAno] = { vendas: 0, valor: 0 };
    }
    
    vendas.forEach(venda => {
      const dataVenda = new Date(venda.document_date);
      ...
      if (vendasPorMes[mesAno]) {
        ...
        vendasPorMes[mesAno].valor += venda.total_amount || 0;
      }
    });
    
    const chartData = Object.entries(vendasPorMes).map(([mes, dados]) => ({
      mes,
      vendas: dados.vendas,
      valor: dados.valor
    }));
...
// GET /api/dashboard/produtos-top - Top produtos mais vendidos
app.get('/api/dashboard/produtos-top', (req, res) => {
  try {
    const data = readData();
    const vendas = data.documents?.filter(doc => 
      doc.document_type === 'FAT' && doc.status === 'SENT'
    ...
    vendas.forEach(venda => {
      if (venda.linhas) {
        venda.linhas.forEach(linha => {
          if (produtosVendidos[linha.codigo]) {
            produtosVendidos[linha.codigo].quantidade += linha.quantidade;
            produtosVendidos[linha.codigo].valor += linha.total;
          } else {
            produtosVendidos[linha.codigo] = {
              codigo: linha.codigo,
...
app.get('/api/dashboard/alertas', (req, res) => {
  try {
    ...
    const produtos = data.produtos || [];
    const produtosBaixoStock = produtos.filter(p => 
      p.stock_atual <= p.stock_minimo && p.stock_atual > 0
    );
    
    produtosBaixoStock.forEach(produto => {
      alertas.push({
        id: `stock_baixo_${produto.id}`,
        tipo: 'warning',
        titulo: 'Stock Baixo',
        mensagem: `${produto.nome} - Apenas ${produto.stock_atual} unidades restantes`,
        data: new Date().toISOString(),
        acao: 'Ver Stock',
        prioridade: 'media'
      });
    ...
    // Alertas de stock esgotado
    const produtosSemStock = produtos.filter(p => p.stock_atual <= 0);
    
    produtosSemStock.forEach(produto => {
      alertas.push({
        id: `stock_esgotado_${produto.id}`,
        tipo: 'error',
        titulo: 'Stock Esgotado',
        mensagem: `${produto.nome} - Stock esgotado`,
        data: new Date().toISOString(),
        acao: 'Repor Stock',
        prioridade: 'alta'
      });
    ...
    // Alertas de orçamentos pendentes
    const orcamentos = data.documents?.filter(doc => 
      doc.document_type === 'ORC' && doc.status === 'DRAFT'
    ) || [];
    ...
    if (orcamentos.length > 0) {
      alertas.push({
        ...
        mensagem: `${orcamentos.length} orçamentos aguardando aprovação`,
        data: new Date().toISOString(),
        acao: 'Ver Orçamentos',
        prioridade: 'baixa'
      });
