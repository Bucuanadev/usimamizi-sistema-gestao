...
const Vendas: React.FC = () => {
  ...
  const loadProdutosStock = async () => {
    try {
      const response = await fetch('/api/sales/produtos');
      if (response.ok) {
        const data = await response.json();
  ...
  const loadCategoriasStock = async () => {
    try {
      const response = await fetch('/api/sales/categorias');
      if (response.ok) {
        const data = await response.json();
  ...
  const loadSucursais = async () => {
    try {
      const response = await fetch('/api/settings/estabelecimentos');
      if (response.ok) {
        const data = await response.json();
  ...
  const buscarProdutos = async () => {
    try {
      ...
      const response = await fetch(`/api/sales/produtos?${params}`);
      if (response.ok) {
        const data = await response.json();
        setProdutosStock(data.data || []);
      }
  ...
  const buscarProdutoPorCodigo = async (codigo: string) => {
    try {
      ...
      const url = `/api/sales/produto-por-codigo/${encodeURIComponent(codigo)}`;
  ...
  const atualizarStock = async (produtos: any[], tipoOperacao: string = 'venda') => {
    try {
      const response = await fetch('/api/sales/atualizar-stock', {
        method: 'POST',
        headers: {
  ...
  const createNewDocument = async (type: string) => {
    ...
    try {
      const response = await fetch(`/api/sales/document-numbers/${type}`);
      ...
      if (data.success) {
        ...
        const createResponse = await fetch('/api/sales/documents', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  const emitirFatura = async () => {
    ...
    try {
      ...
      if (!documentId) {
        const saveResponse = await fetch('/api/sales/documents', {
          method: 'POST',
      ...
      const emitResponse = await fetch(`/api/sales/documents/${documentId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          status: 'SENT',
  ...
  const guardarFatura = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(faturaData.id ? `/api/sales/documents/${faturaData.id}` : '/api/sales/documents', {
  ...
  const exportarPDF = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/sales/export-pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const anularFatura = async () => {
    ...
    if (window.confirm('Tem certeza que deseja anular esta fatura? Esta ação não pode ser desfeita.')) {
      setLoading(true);
      try {
        const response = await fetch(`/api/sales/documents/${faturaData.id}/cancel`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' }
        });
  ...
  const expedirGR = async () => {
    ...
    if (window.confirm('Tem certeza que deseja expedir esta Guia de Remessa?')) {
      setLoading(true);
      try {
        const response = await fetch(`/api/sales/documents/${currentDocument.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            status: 'SENT',
  ...
  const imprimirGR = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/sales/export-gr-pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...currentDocument,
  ...
  const imprimirOrcamento = async () => {
    if (!currentDocument) {
      console.error('currentDocument não está definido');
      alert('Erro: Documento não selecionado');
      return;
    }

    console.log('Iniciando geração de PDF...');
    console.log('currentDocument:', currentDocument);
    console.log('cotacaoItens:', cotacaoItens);
    ...
    try {
      ...
      console.log('Dados da requisição:', requestData);
        ...
        const response = await fetch('/api/sales/export-orc-pdf', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        const errorText = await response.text();
        console.error('Erro na resposta:', response.status, errorText);
        alert(`Erro ao gerar PDF do orçamento: ${response.status} - ${errorText}`);
      }
    } catch (error) {
      ...
      if (error instanceof Error) {
        ...
        console.error('Erro desconhecido:', error);
        alert('Erro ao gerar PDF do orçamento: Erro desconhecido');
      }
    }
  };
